...
class PhotosQueryset(object):
    ...
    def get_photos_queryset(self, request):
        if not request.user.is_authenticated:  # si no esta autenticado
            ...
            photos = Photo.objects.filter(Q(owner=request.user) | Q(visibility=PUBLIC))  # si esta autenticado
            #objeto query con la query configurada
        return photos
...
class HomeView(View):#Vista basada en clase
    ...
    def get(self, request):
        #django solo se trae de la bd las primeras 5, aunque parezca que se trae todo
...
class DetailView(View, PhotosQueryset):
    ...
    def get(self, request, pk):
        ...
        :return: HttpResponse
        """
        possible_photos = self.get_photos_queryset(request).filter(pk=pk).select_related('owner') #select_related hace inner join con auth_user. Es decir, se trae la foto y su propietario
        #PhotosQueryset.get_photos_queryset(request) obtiene fotos segun su autenticacion
...
class CreateView(View):
    ...
    # @login_required() para vistas basadas en funciones
    @method_decorator(login_required())
...
class PhotoListView(View, PhotosQueryset):
    ...
    def get(self, request):
        """
        Devuelve:
        - Si el usuario no esta autenticado, las fotos publicas
        - Si el usuario esta autenticado, las suyas y las publicas de otros
